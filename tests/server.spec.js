...
describe('GET /v1/universidad/tipo/:tipo', () => {
    test('should respond with a 200 status code', async () => {
        const response = await request(app).get('/v1/universidad/tipo/1').send();
        expect(response.status).toBe(200);
    })

    test('should respond with a 400 status code', async () => {
        const response = await request(app).get('/v1/universidad/tipo/as').send();
        expect(response.status).toBe(400);
    })

    test('should respond with a 500 status code', async () => {
        const response = await request(app).get('/v1/universidad/tipo/3').send();
        expect(response.status).toBe(500);
    })
